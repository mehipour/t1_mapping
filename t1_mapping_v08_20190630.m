...
manual_c_segmentation = 0;   % if 1 then can manually segment c image
...
if pre
    ...
    if or(~data_available,fit_override)
        ...
        if manual_R1_segementation
            segment_then_find_mean_and_sd_v01_20190630(pre_r1_map,pre_path,pre_file_name,'R1 (1/s)');
        end
    end
    % show pre R1 maps
...
if post
    ...
    if or(~data_available,fit_override)
        ...
        if manual_R1_segementation
            segment_then_find_mean_and_sd_v01_20190630(post_r1_map,post_path,post_file_name,'R1 (1/s)');
        end
    end
    % show pre R1 maps
